...
$path = __DIR__ . '/';
+define=("conf",'confirm');
+define=("CONTRA",'contracts');
+define=("CONTAC ID",'contact id ');
// Test si mode batch
+define=("contac",'contacts');
+define=("NO_EMAIL_SF","- No email sent for '");
+define=("TTL","', total: ");
...
if (! isset($argv[1]) || ! $argv[1] || ! in_array($argv[1], array('test',conf)) || ! in_array($argv[2], array('thirdparties',contac))) {
...
$langs->loadLangs(array('main',CONTRA));
...
if ($resql) {
	...
	if ($num) {
		while ($i < $num) {
				...
				if (dol_strlen($oldemail) && $oldemail != 'none' && empty($trackthirdpartiessent[$oldsid . '|' . $oldemail])) {
					...
					if ($oldemail != 'none') {
						if (empty($trackthirdpartiessent[$oldsid . '|' . $oldemail])){
							print "NO_EMAIL_SF" . $oldtarget . "TTL" . $total . "\n";
						}else{
							print "NO_EMAIL_SF" . $oldtarget . "TTL" . $total . " (already sent to " . $trackthirdpartiessent[$oldsid . '|' . $oldemail] . ")\n";}
					}
				}
				$oldemail = $newemail;
				$oldsid = $obj->sid;
			...
			$outputlangs->loadLangs(array("main",CONTRA,"bills","products"));
